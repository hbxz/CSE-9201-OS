	    ...
	    u_int32_t *diskblock)
{
	...
	assert(sizeof(blockbuf)==SFS_BLOCKSIZE);
	...
	if (fileblock < SFS_NDIRECT) {
		...
		if (block==0 && doalloc) {
			...
			sv->sv_i.sfi_blocks++; // fk: increase blocknumbers
	...
	/* 
	 * check if fileblock is out of range (1 or bigger when 
	 * devided on total number of block - NDIRECT blocks)
	 */
	idnum = fileblock / (SFS_DBPERIDB + SFS_DBPERIDB*SFS_DBPERIDB + SFS_DBPERIDB*SFS_DBPERIDB*SFS_DBPERIDB);


	/*
	 * If the offset we were asked for
	...
	/*
	 * ********************************************************************
	 * Main changes of assignment 3 goes here. This changes is done to
	 * support double and triple indirect blocks, in addition to indirect
	 * blocks.
	 * ********************************************************************
	 */

	/*
	 * Calculate what indirect level of blocks we have to use:
	 * idblevel = 1 - indirect, 2 - double, 3 - triple
	 */
	...
	/*
	 * Select the level 0 block for indirect/double/triple.
	 * Then modify "blockno" so it's the offset into the indirect space
	 */
	if (idblevel == 1){
		idblock = sv->sv_i.sfi_indirect;
	}else if (idblevel == 2){
		fileblock -= SFS_DBPERIDB;
		idblock = sv->sv_i.sfi_doubleindirect;
	}else if (idblevel == 3){
		fileblock -= (SFS_DBPERIDB*SFS_DBPERIDB-SFS_DBPERIDB);
		idblock = sv->sv_i.sfi_tripleindirect;
	}

	/* 
	 * If we're asked not to allocate and there is no indirect block
	 * then we return a diskblock with zero
	 */
	...
	/* 
	 * otherwise we load the content from the indirect block into blockbuffer
	 */
	else {
		result = loadbuffer(idblock,0);
		if (result)
			return result;
	}
	...
	/*
	 * Overview over a triple indirect block buffer, and the levels:
	 *
	 *  level 0			level 1			level 2			level 3
	 * [idblock] ----> [idblock] ----> [idblock] ----> [datablock]
	 *
	 * Indirect blocks has only level 0 w/indirect blocks, and level 1
	 * w/datablocks.
	 * Double indirect blocks has level 0 and 1 w/idblocks, level 2 
	 * w/datablocks.
	 */
	...
	/*
	 * indirect block:
	 * the buffer is now loaded with all datablocks in level 1
	 * so all we got to do is to get the right block from the buffer
	 */
	if (idblevel == 1){

		/* calculate the right blocknumber */
		blockno = fileblock%SFS_DBPERIDB;

		/* select block out of blockbuffer into "block" */
		result = get_block(blockno);
		if (result)
			return result;
	}
	/* 
	 * double indirect block: 
	 * first we have to get the right block from level 1, and since
	 * this is a indirect block, we have to re-load the buffer with
	 * level 2 datablocks, and then return the right block
	 */
	else if (idblevel == 2){
		...
		/* get block out of blockbuffer */
		result = get_block(blockno);
		if (result)
			return result;
		
		/* since "block" now actually is a indirect block, we have to update "idblock" */
		idblock = block;
		
		/* resets buffer (it containe data from level 1 idblock-load) */
		bzero(blockbuf, sizeof(blockbuf));
		
		/* 
		 * load the buffer again - with info from idblock in level 1 -
		 * which is the actual datablocks in level 2 (1 as an argument 
		 * prevents the double indirect inode pointer to be updated) 
		 */
		result = loadbuffer(block,1);
		if (result)
			return result;

		/* re-calculate to find the right blocknumber for the datablock */
		blockno = fileblock%SFS_DBPERIDB;

		/* get the result out from buffer and into "block" */
		result = get_block(blockno);
		if (result)
			return result;
	}
	 ...
	 * Almost the same as for double, but have to load one
	 * more indirect block before we get to the datablocks.
	 */
	else if (idblevel == 3){

		/* calculate to find level 1 blocknumber */
		blockno = fileblock/(SFS_DBPERIDB*SFS_DBPERIDB);

		/* fetch the level 1 block from blockbuffer into "block" */
		result = get_block(blockno);
		if (result)
			return result;

		/* since the result is an indirect block, update "idblock" */
		idblock = block;

		/* reset blockbuffer */
		bzero(blockbuf, sizeof(blockbuf));

		/* load the buffer with data (level 2 id-blocks) from "block" */
		result = loadbuffer(block,1);
		if (result)
			return result;

		/* re-calculate the blocknumber for level 2 id-block */
		blockno = fileblock/SFS_DBPERIDB;

		/* fetch the block from buffer into "block" */
		result = get_block(blockno);
		if (result)
			return result;

		/* since also this result is an indirect block, update "idblock" */
		idblock = block;

		/* reset blockbuffer (contains level 2 id-blocks) */
		bzero(blockbuf, sizeof(blockbuf));

		/* load the buffer with data (level 3 datablocks) from "block" */
		result = loadbuffer(block,1);
		if (result)
			return result;

		/* calulate the blocknumber for the datablock */
		blockno = fileblock%SFS_DBPERIDB;

		/* fetch datablock out of buffer and into "block" */
		result = get_block(blockno);
		if (result)
			return result;
	}
...
sfs_reclaim(struct vnode *v)
{
	...
	if (sv->sv_i.sfi_linkcount==0) {
		...
		sv->sv_i.sfi_blocks--; // fk: decrease blocknumbers
	}
...
sfs_stat(struct vnode *v, struct stat *statbuf)
{
	...
	statbuf->st_nlink = 0;
	statbuf->st_blocks = sv->sv_i.sfi_blocks; // fk 
...
sfs_truncate(struct vnode *v, off_t len)
{
	...
	if (blocklen < highblock && idblock != 0) {
		...
		if (!hasnonzero) {
			...
			if (sv->sv_i.sfi_doubleindirect!=0){
				sv->sv_i.sfi_blocks--;
				sv->sv_i.sfi_doubleindirect = 0;
			}
			if (sv->sv_i.sfi_tripleindirect!=0){
				sv->sv_i.sfi_blocks--;
				sv->sv_i.sfi_tripleindirect = 0;
			}
			sv->sv_dirty = 1;
		}
		else if (iddirty) {
